...
var charts = 
{
	...
	chart_donut:
	{
		// chart data
		data: [
		    { label: "Dania glowne",  data: 38 },
		    { label: "Sniadania",  data: 23 },
		    { label: "Przystawki",  data: 15 },
		    { label: "Zupy",  data: 9 },
		    { label: "Napoje",  data: 7 },
		    { label: "Desery",  data: 5 },
		    { label: "Przystawki",  data: 3 }
		],
	...
	chart_horizontal_bars:
	{
		...
		init: function()
		{
			...
			var d1 = [];
		    for (var i = 0; i <= 5; i += 1)
		        d1.push([parseInt(Math.random() * 100),i ]);

		    var d2 = [];
		    for (var i = 0; i <= 5; i += 1)
		        d2.push([parseInt(Math.random() * 100),i ]);

		    var d3 = [];
		    for (var i = 0; i <= 5; i += 1)
		        d3.push([ parseInt(Math.random() * 100),i]);
