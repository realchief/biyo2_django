...
//    $('[name$="date"]').datetimepicker(datepickerConfig);
    ...
    var OrderForm = function(){
        ...
        this.instance = $('#instance_id').val();
        this.instance_input = $('#id_order_id');  //TODO: perhaps obsolete
        this.form_selector = '#update-order-form';
        ...
        this.payments = null;
        this.customerPrices = {};
        this.items_formset = '#order-items-formset';
        this.counter = 0;
        this.initialize();
    };
    ...
    OrderForm.fn.attachDiscounts = function(){
        ...
        OrderForm.fn.recalculateItems = function(form){
        (function(self){
            $('#subtotal').text(0.00);
            // $('#discounts').text(0.00);
            // $('#taxes').text(0.00);
            $(form).find('tr').slice(2).each(function (index, element) {
                var item_quantity =  Number($(form).find("#" + element.id).find('#id_quantity').val());
                var item_discount = Number($(form).find("#" + element.id).find('#id_discount').val()) * item_quantity;
                var item_tax = Number($(form).find("#" + element.id).find('#id_tax').val()) * item_quantity;
                var discount = $('#discounts').text();
                var taxes = $('#taxes').text();

                var modifiersPrice = 0;
                $(element).find('.modifiers_list li').each(function(index, item) {
                   modifiersPrice += +$(item).find('span').text().replace('$', '');
                });

                var item_subtotal = (($(form).find("#" + element.id).find('#id_price').val() * item_quantity) - item_discount + modifiersPrice + item_tax).toFixed(2);
                var main_subtotal = $('#subtotal').text();
                $(form).find("#" + element.id).find('.item-subtotal').text(item_subtotal);
                $('#subtotal').text((Number(item_subtotal) + Number(main_subtotal)).toFixed(2));
            });
        })(this);
    };
    ...
    OrderForm.fn.recalculate = function(){
        ...
        var discount = this.calculateDiscount('#id_discount_total', '#id_subtotal');
        ...
        if ($(this.items_formset).find('tr').length > 0 & this.counter == 0) {
            if ($(this.items_formset).find('tr')[2]) {
                $(this.items_formset).find('#' + $(this.items_formset).find('tr')[2].id).find('#id_quantity').trigger("change");
            }
            this.counter += 1
        }
        if ($(this.items_formset).find('tr').length > 0) {
            this.recalculateItems(this.items_formset);
        }
    };
                ...
                if(self.instance){
                    ...
                    $.ajax({
                        ...
                        success: function(data){
                            ...
                            // else {
                            //     debugger;
                            //     self.formset.formset = $(data).find('#order-items-formset');
                            // }
                            self.recalculate();
                        },
    ...
    var OrderModifier = function(data, form, parent, instanceId){
        this.form = $(form);
        this.data = data;
        this.formset = parent;
        this.instance = instanceId || this.form.find('#id_instance_id').val();
        var _form;
    ...
    OrderModifierForm.fn.showModal = function(){
        (function(self){
            ...
            // self.modal.find('.modal-body').append('<div class="hidden" id="template" style="display: none">');
            var childrens = self.modal.find('.modal-body').children();
    ...
    OrderModifierForm.fn.addForm = function(data, withoutSave, instanceId){
        data = data || {};
        (function(self, instance){
            var form = self.order_modifier_template.clone();

            var modifier = new OrderModifier(data, form, self, instance);
            if (withoutSave) {
                form.find("h4+h3.price-cost").text("$" + data.price.toFixed(2));
                form.addClass('alert-info');
            }
    ...
    OrderModifierForm.fn.lookupModifiers = function(){
        (function(self){
            ...
            var modifiers_list = self.ancestor.form.find("td .modifiers_list");
             if (modifiers_list.length == 0) {
                self.ancestor.form.find("td.span3").append("<div class='modifiers_list'></div>");
                modifiers_list = self.ancestor.form.find(".modifiers_list");
            }
    ...
    var OrderItemForm = function(table_row, ancestor){
            ...
            this.product_id= this.form.find('input[name$="product_id"]').val();
            this.processing = new $.Deferred();
        ...
        OrderItemForm.fn.initModifierList = function () {
            var prodId = $(this.form).find('input[name="product_id"]').val();
            ...
            if (product)
                this.modifierFormset = new OrderModifierForm(product, this);
            else
                this.modifierFormset = null;
        };

        OrderItemForm.fn.initSelectedModifiers = function () {
            var products = JSON.parse(localStorage.getItem("products"));
            if (products && products.length > 0) {
                var self = this;
                this.selectedModifiers = [];
                this.selectedModifiersIds = [];
                this.form.find('.modifiers_list li').each(function(index, item) {
                    var selectedModifier = $(item);
                    self.selectedModifiers.push({
                        instance_id: selectedModifier.data('itemModifier'),
                        id: selectedModifier.data('modifier'),
                        name: selectedModifier.data('name')
                    });
                    self.selectedModifiersIds.push(selectedModifier.data('modifier'));
                });

                var currentProduct = products.filter(function (item) {
                    return item.id === +self.product_id;
                });

                if (currentProduct[0] && currentProduct[0].modifier_groups)
                    currentProduct[0].modifier_groups.forEach(function (group) {
                        group.modifiers.forEach(function(modifier){

                            var index = self.selectedModifiersIds.indexOf(modifier.id);
                            if (index > -1) {
                                self.selectedModifiers.forEach(function (item) {
                                    if (item.id === modifier.id && self.selectedModifiersIds.indexOf(modifier.id) > -1) {
                                        self.selectedModifiersIds.splice(index, 1);
                                        modifier.name = item.name;
                                        modifier.group = group.id;
                                        modifier.item = self.instance;
                                        self.modifierFormset.addForm(modifier, true, item.instance_id);
                                    }
                                 });
                            }
        ...
        OrderItemForm.fn.createOrUpdate = function(data, callback){
            (function(self){
                self.processing.done(function(){
                    ...
                    if(data.item_id){
                        url += 'update/order/item/' + data.item_id;
                        method = 'PUT';
                    } else {
                        url += 'add/order/item';
                        method = 'POST';
    ...
    OrderItemsFormset.fn.initialize = function(html_fragment){
        ...
        this.order_form.recalculate($('#update-order-form'));
        (function (self) {
            $.ajax({
    ...
    PaymentHandler.fn.initialize = function(){
        (function(self){
            self.button = $("<button class='btn btn-success waves-effect pull-md-right'><i class='icon md-shopping-cart'></i>Checkout</button>");
